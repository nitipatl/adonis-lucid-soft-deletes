...
const defineModel = (name, deletedAtColumn, lucid, bootCallback = noop, extendObject = {}) => {
  const Model = class extends lucid.Model {
    static boot () {
      super.boot()

      this.addTrait(...['@provider:Lucid/SoftDeletes', customOptions[deletedAtColumn]])
...
Object.keys(['deleted_at']).forEach(deletedAtColumn => {
  describe(`softDeletes ${deletedAtColumn}`, () => {
    ...
    beforeEach(async () => {
      ...
      User = defineModel('User', deletedAtColumn, lucid)
    })
    ...
    it('marks model as deleted', async () => {
      ...
      expect(+model[deletedAtColumn]).to.equal(+new Date())
    ...
    it('restores deleted model', async () => {
      const user = await User.create({
        username: 'Jon',
        [deletedAtColumn]: moment().format(DATE_FORMAT)
      })

      await user.restore()

      const check = await lucid.db.table('users')
        .where({ id: user.id, [deletedAtColumn]: null })
        .first()
    ...
    it('does not query deleted records', async () => {
      await Promise.all([
        ...
        User.create({ username: 'Ed', [deletedAtColumn]: moment().format(DATE_FORMAT) })
      ])
    ...
    it(`adds ${deletedAtColumn} column to dates`, () => {
      expect(User.dates).to.eql(['created_at', 'updated_at', deletedAtColumn])
    })
    ...
    describe('events', () => {
      it('triggers delete hooks', async () => {
        ...
        const User = defineModel('User', deletedAtColumn, lucid, function () {
    ...
    describe('Many-to-many relation', () => {
      ...
      before(() => {
        ...
        User = defineModel('User', deletedAtColumn, lucid, noop, {
          methods: {
            tags () {
        ...
        Tag = defineModel('Tag', deletedAtColumn, lucid)
        UserTag = defineModel('UserTag', deletedAtColumn, lucid)
      ...
      it('does not include deleted pivots', async () => {
        ...
        await lucid.db.table('user_tags')
          ...
          .update({ [deletedAtColumn]: new Date() })
    ...
    describe('Regression tests', () => {
      it('gh-10', async () => {
        const Comment = defineModel('Comment', deletedAtColumn, lucid, noop, {
          methods: {
            replies () {
        ...
        await Promise.all([
          ...
          Comment.create({ reply_to_comment_id: comment.id, [deletedAtColumn]: new Date() }),
          Comment.create({ reply_to_comment_id: comment.id, [deletedAtColumn]: new Date() })
        ])
